...
func Handler(ctx context.Context, request Request) (Response, error) {
	var projectCategoryID = request.PathParameters["projectCategoryID"]
	...
	err = json.Unmarshal([]byte(request.Body), &project)
	if err != nil {
		log.Println(err)
		return Response(utils.APIGateway500(errors.New("error unmarshalling"))), nil
	}

	err = database.CreateProject(db, projectCategoryID, project)
	if err != nil {
		log.Println(err)

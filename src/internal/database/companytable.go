...
func UpdateCompany(db *sqlx.DB, companyID string, data types.Company) error {
	_, err := db.Exec("UPDATE companies SET name = $2, description = $3, "+
		"logo = $4, bg_color = $5 WHERE id = $1", companyID, data.Name, data.Description, data.Logo, data.BgColor)
	if err != nil {
...
func GetCompanyByID(db *sqlx.DB, companyID string) (types.Company, error) {
	...
	err := db.Get(&company, "SELECT name, "+
		"description, logo, bg_color FROM companies WHERE id = $1",
		companyID)
...
func GetAllCompanies(db *sqlx.DB) ([]types.Company, error) {
	...
	err := db.Select(&companies, "SELECT name, description, logo, "+
		"bg_color FROM companies WHERE deleted_at is null")
